...
def standard_metrics(y_train, y_test, test_logits, verbose=False):
    if len(y_test) != len(test_logits):
        y_test = y_test[:len(test_logits)]
    ...
    threshold = threshold
    y_prob = np_softmax(test_logits[:,1], test_logits)
    y_pred = np.where(y_prob > threshold, 1, 0)
    
    auroc = roc_auc_score(y_test, test_logits[:,1])
    auprc = average_precision_score(y_test, test_logits[:,1])
